...
def pose_callback(data):
    global current_pose, last_pose, target_detected
    current_pose = data

    if target_detected is None:
        target_detected = rospy.wait_for_message("/states", States)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        control_msg.type_mask = PositionTarget.IGNORE_VX + PositionTarget.IGNORE_VY + PositionTarget.IGNORE_VZ + PositionTarget.IGNORE_AFX + PositionTarget.IGNORE_AFY + PositionTarget.IGNORE_AFZ + PositionTarget.IGNORE_YAW + PositionTarget.IGNORE_YAW_RATE

        if current_pose is None or target_detected is None:
            rate.sleep()
        elif target_detected.Xc == 0.0 and target_detected.Yc == 0.0 and target_detected.W == 0.0:
            if last_pose is not None:  # last_pose가 None인지 확인
                rospy.wait_for_message("/state")
                rospy.loginfo("No Vertiport! Chasing last one!")
                setpoint_pub.publish(control_msg)
            else:
                rospy.logwarn("last_pose is None, cannot publish setpoint.")
        elif (3 < current_pose.pose.position.z) or (current_pose.pose.position.z < 1):
            here_pose = PoseStamped()
            here_pose.pose.position.x = current_pose.pose.position.x
            here_pose.pose.position.y = current_pose.pose.position.y
            
            start_time = rospy.Time.now()
            duration = rospy.Duration(0.5)  # 0.5초 동안 발행

            while rospy.Time.now() - start_time < duration:
                control_msg.position.x = here_pose.pose.position.x
                control_msg.position.y = here_pose.pose.position.y
                control_msg.position.z = 2
                rospy.loginfo("z is so noised! Correction Working!")
                setpoint_pub.publish(control_msg)
                rate.sleep()
            ...
            if not target_reached:
                ...
                error_x = target_x - image_center_x
                error_y = target_y - image_center_y
                if abs(theta_rad) < math.radians(2):  # Theta가 1도 이하일 때 정렬 완료
                    yaw_aligned = True
