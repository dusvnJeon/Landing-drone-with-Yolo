...
def main():
    global target_reached, yaw_aligned, final_adjustment_done, control_msg, landing
    ...
    while not rospy.is_shutdown():
        ...
        if target_detected.Xc == 0.0 and target_detected.Yc == 0.0 and target_detected.W == 0.0:
            if control_msg != None:
                if landing == True:
                    ...
                    rospy.loginfo("Vertiport is too closed! Keep landing!")  
                else:
                    setpoint_pub.publish(control_msg)
                    rospy.loginfo("No Vertiport!")
            else:
                rospy.wait_for_message('/states', States)
                rospy.loginfo("No Vertiport! Wait for '/state'!")
            continue
        ...
        elif final_adjustment_done and not landing:
                control_msg.velocity.x = 0  # y축 오차를 x축 속도로 변환
                control_msg.velocity.y = 0  # x축 오차를 y축 속도로 변환
                control_msg.velocity.z = -0.5  # 고도 유지

        setpoint_pub.publish(control_msg)
        
        # 착륙 완료 조건 체크 (고도 < 0.1미터)
        if landing and current_pose.pose.position.z < 0.1:
...
2. 맨처음에 버티포트가 보이지 않으면 control_msg가 생성되지 않아서 코드가 실행되지 않음.
    - control_msg가 생성되지 않으면, 그냥 wait_for_message하는걸로 변경
