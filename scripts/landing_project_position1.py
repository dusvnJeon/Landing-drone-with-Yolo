...
aligned_pose = None
aligned_pose_flag = None

def pose_callback(data):
    global aligned_pose
    global aligned_pose_flag
    if target_reached == True and aligned_pose == None:
        aligned_pose = data
        aligned_pose_flag = True
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        control_msg.type_mask = PositionTarget.IGNORE_PX + PositionTarget.IGNORE_PY + PositionTarget.IGNORE_VZ + PositionTarget.IGNORE_AFX + PositionTarget.IGNORE_AFY + PositionTarget.IGNORE_AFZ + PositionTarget.IGNORE_YAW
        ...
        if not target_reached:
            ...
            if abs(error_x) < 10 and abs(error_y) < 10:
                ...
                control_msg.velocity.x = -(error_y * 0.0075)  # y축 오차를 x축 속도로 변환
                control_msg.velocity.y = -(error_x * 0.0075)  # x축 오차를 y축 속도로 변환
                control_msg.position.z = maintain_z_position  # 고도 유지
            ...
            if abs(theta_rad) < math.radians(1):  # Theta가 1도 이하일 때 정렬 완료
                ...
                # control_msg.position.x = aligned_pose.pose.position.x  # y축 오차를 x축 속도로 변환
                # control_msg.position.y = aligned_pose.pose.position.y  # x축 오차를 y축 속도로 변환
                control_msg.velocity.x = 0  # 유지
                control_msg.velocity.y = 0  # 유지
                control_msg.position.z = maintain_z_position # 고도 유지
                control_msg.type_mask = PositionTarget.IGNORE_PX + PositionTarget.IGNORE_PY + PositionTarget.IGNORE_VZ + PositionTarget.IGNORE_AFX + PositionTarget.IGNORE_AFY + PositionTarget.IGNORE_AFZ + PositionTarget.IGNORE_YAW
