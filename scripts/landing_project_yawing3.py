...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        control_msg.type_mask = PositionTarget.IGNORE_PX + PositionTarget.IGNORE_PY + PositionTarget.IGNORE_PZ + PositionTarget.IGNORE_AFX + PositionTarget.IGNORE_AFY + PositionTarget.IGNORE_AFZ + PositionTarget.IGNORE_YAW_RATE
        ...
        if not target_reached:
            ...
            # 현재 yaw 값을 가져와서 목표 yaw 값과 비교
            current_yaw = get_current_yaw()
            target_yaw = math.radians(target_detected.Theta)
            yaw_error = target_yaw - current_yaw
            
            if abs(yaw_error) < math.radians(1):  # yaw 오차가 1도 이하일 때 정렬 완료
                yaw_aligned = True
                rospy.loginfo("Yaw aligned. Beginning descent.")
            else:
                # yaw_rate에 비례 상수를 곱하여 제어 명령 생성
                control_msg.yaw_rate = -yaw_error * 0.1  # 비례 상수 0.1 적용
            ...
            if current_pose.pose.position.z > 2.5:
                ...
                control_msg.velocity.x = - (error_y * 0.01)
                control_msg.velocity.y = - (error_x * 0.01)
